...
namespace matplot{ namespace backend {
    ...
    bool
    backend_interface::output(const std::string &filename) {
    ...
    void
    backend_interface::run_command(const std::string &text) {
        if (consumes_gnuplot_commands()) {
            throw std::logic_error(
    ...
    void backend_interface::draw(matplot::figure_type* fig)
    {
        ...
        // for gnuplot
        if (!this->consumes_gnuplot_commands())
        {
            fig->send_draw_commands();
        }
        else
        {
            fig->send_gnuplot_draw_commands();
        }
    }
    ...
    void
    backend_interface::window_title(const std::string &title) {
    }
