...
namespace matplot {
    ...
    using TypeConcept = type_concept<T, typename std::enable_if<C>::type>;
    ...
    template <typename T>
    using String = TypeConcept<T, std::is_same<typename std::decay<T>::type, std::string>::value>;

    // TypeConcept where C is true only if T is NotString
    template <typename T>
    using NotString =
        TypeConcept<T, !std::is_same<typename std::decay<T>::type, std::string>::value>;

    // TypeConcept where C is true only if T is NotString
    template <typename T>
    using NotStringConvertible =
        TypeConcept<T, !std::is_convertible<typename std::decay<T>::type, std::string>::value>;

    class figure_type;
    // TypeConcept where C is true only if T is FigureHandle
    template <typename T>
    using FigureHandle = TypeConcept<
        T, std::is_same<typename std::decay<T>::type, std::shared_ptr<class figure_type>>::value>;

    // TypeConcept where C is true only if T is NotFigureHandle
    template <typename T>
    using NotFigureHandle =
        TypeConcept<T, !std::is_same<typename std::decay<T>::type,
                                       std::shared_ptr<class figure_type>>::value>;

    // TypeConcept where C is true only if T is Iterable
    template <typename T>
    using Iterable = TypeConcept<T, is_iterable<typename std::decay<T>::type>::value>;

    // TypeConcept where C is true only if T is IterableValues
    template <typename T>
    using IterableValues = TypeConcept<T, is_iterable_value<typename std::decay<T>::type>::value>;

    // TypeConcept where C is true only if T is IterablePair
    template <typename T>
    using IterablePairs = TypeConcept<T, is_iterable_pair<typename std::decay<T>::type>::value>;

    // TypeConcept where C is true only if T is IterableIterables
    template <typename T>
    using IterableIterables =
        TypeConcept<T, is_iterable_iterable<typename std::decay<T>::type>::value>;

    // TypeConcept where C is true only if T is IterableValues
    template <typename T>
    using NotAxesHandle =
        TypeConcept<T, !std::is_same<typename std::decay<T>::type, axes_handle>::value>;
